...
import com.example.website_ban_ao_the_thao_psg_du_an_tot_nghiep.model.mapper.GioHangMapper;
...
import com.example.website_ban_ao_the_thao_psg_du_an_tot_nghiep.repository.GioHangRepository;
...
public class KhachHangServiceImpl implements KhachHangService {
    ...
    public byte[] exportExcelKhachHang() throws IOException {
        ...
        for (KhachHang khachHang : data) {
            ...
            row.createCell(0).setCellValue(khachHang.getId());
            row.createCell(1).setCellValue(khachHang.getThuHang().getId());
            row.createCell(2).setCellValue(khachHang.getMa());
            row.createCell(3).setCellValue(khachHang.getTen());
            row.createCell(4).setCellValue(khachHang.getAnh());
            row.createCell(5).setCellValue(khachHang.getGioiTinh() != null ? khachHang.getGioiTinh().booleanValue() : false);
            row.createCell(6).setCellValue(String.valueOf(khachHang.getNgaySinh()));
            row.createCell(7).setCellValue(khachHang.getDiaChi());
            row.createCell(8).setCellValue(khachHang.getSdt());
            row.createCell(9).setCellValue(khachHang.getEmail());
            row.createCell(10).setCellValue(khachHang.getMatKhau());
            row.createCell(11).setCellValue(khachHang.getSoLuongDonHangThanhCong());
            row.createCell(12).setCellValue(String.valueOf(khachHang.getSoTienDaChiTieu()));
            row.createCell(13).setCellValue(String.valueOf(khachHang.getSoDiem()));
            row.createCell(14).setCellValue(String.valueOf(khachHang.getNgayTao()));
            row.createCell(15).setCellValue(String.valueOf(khachHang.getNgayCapNhat()));
            row.createCell(16).setCellValue(String.valueOf(khachHang.getTrangThai()));
        }
...
//            }
//
//            if(khachHangRepository.findByEmail(khachHang.getEmail()) != null){
...
//            customEmailSender.signupKhachHangSendEmail(khachHang);
            ...
            throw new InternalServerException("Failed to create khach hang due to an unexpected error." + ex);
        }
    }
...
//            }
            ...
            throw new InternalServerException("Failed to update khach hang due to an unexpected error." + ex);
        }
    }
